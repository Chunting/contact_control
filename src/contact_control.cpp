...
std::future <Contact::EndCondition>
ContactControl::balanceAsync(Contact::Dimension lDim, Contact::Dimension fDim, bool inWorld, double leverArm,
                             Contact::Dimension mDim, double mSpeed, double fMax, double tMax) {
  return std::async(std::launch::async, &ContactControl::balance, this, lDim, fDim, inWorld, leverArm, mDim, mSpeed,
                    fMax, tMax);
}
...
void ContactControl::toControlFrame(geometry_msgs::WrenchStamped &data) {
  if (data.header.frame_id.compare(controlFrame) != 0) {
    ...
    if (listener->waitForTransform(controlFrame, data.header.stamp, data.header.frame_id, data.header.stamp,
                                   fixedFrame, ros::Duration(1.0))) {
...
void
ContactControl::setSpring(Contact::Dimension dim, double k, double b, double posOffset, double dMax, double ftMax) {
...
void ContactControl::getFT(Contact::Perspective perspective, double &ft, ros::Time &time) {
  ...
  switch (perspective) {
    case Contact::TRANSLATION:
      ft = pow(pow(ftData.wrench.force.x, 2) + pow(ftData.wrench.force.y, 2) + pow(ftData.wrench.force.z, 2),
               0.5);
      break;
    case Contact::ROTATION:
      ft = pow(pow(ftData.wrench.torque.x, 2) + pow(ftData.wrench.torque.y, 2) + pow(ftData.wrench.torque.z, 2),
               0.5);
      break;
    default:
